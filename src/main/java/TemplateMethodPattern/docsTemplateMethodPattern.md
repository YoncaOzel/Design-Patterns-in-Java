...
```
          ...
          │ (limon her zaman)  │      │ Hook: sorgulanır   │
          └────────────────────┘      └────────────────────┘
```
...
└────────────┬────────────────┘
             │
             ├─► boilWater()           [Concrete - Tüm alt sınıflar]
             │
             ├─► brew()                [Abstract - Her alt sınıf farklı]
             │   ├─ Tea: Steeping
             │   └─ Coffee: Dripping
             │
             ├─► pourInCup()           [Concrete - Tüm alt sınıflar]
             │
             └─► if (customerWantsCondiments())  [Hook - İsteğe bağlı]
                 ├─ Tea: true → Always add Lemon
                 └─ Coffee: Ask → Add Sugar/Milk if Yes
```
